...
function hasDuplicate(nums) {
  // Set 自动去重，若长度不相等则存在重复
  return new Set(nums).size !== nums.length;
}
//给定起点和终点，绘制线条和相关坐标点的元素
function drawLine(start, end, svgbox, information, currentHeightList) {
  //   console.log("drawLine", information, currentHeightList);
  ...
  //默认展开，绘制减号
  let isFolded = false;
  let path_d_rect = `M${start[0] - POINT_WIDTH / 2 + 2} ${
    start[1] + POINT_WIDTH / 2
  } 
         L${start[0] + POINT_WIDTH / 2 - 2} ${start[1] + POINT_WIDTH / 2}`;
  //起始点和终点在同一位置，判定为折叠，对应的数组里面有隐藏行也视为折叠（比如起始位置为0，结束位置为10，中间有一行隐藏整体也算折叠）,添加加号,否则为展开，添加减号
  const isAllFolded = end[1] - start[1] < 3;
  if (isAllFolded || hasDuplicate(currentHeightList)) {
    isFolded = true;
  ...
  !isAllFolded && svgbox.appendChild(path);
...
function dragGroupsLine(svgbox, rowGroupData) {
  ...
  //从每行的上边界开始算，所以补充一个0在最前面
  const row_height_list = [0, ...Store.visibledatarow];
  ...
  function drawLinItem(group, currentDepth) {
    ...
    const endIndex =
      group.endIndex > maxRowIndex ? maxRowIndex : group.endIndex;
    const start = row_height_list[startIndex];
    const end = row_height_list[endIndex];
    const item_hide_list = row_height_list.slice(startIndex, endIndex);
    ...
    drawLine(startPoint, endPoint, svgbox, group, item_hide_list);
  }
  ...
  $("#luckysheet-row-groups")
    ...
    .on("click", "rect", function (e) {
      ...
      if (foldstatus == "opened") {
        //折叠
        hideRowOrColumn("row", startIndex, endIndex, {
          order: infor.sheetOrder,
        });
        // $(this).attr("foldstatus", "folded");
      } else {
        //展开
        showRowOrColumn("row", startIndex, endIndex, {
          order: infor.sheetOrder,
        });
...
export function getCurrentGroupsData() {
  return Store.rowGroups;
}
//设置当前分组数据
export function setCurrentGroupsData(data) {
  Store.rowGroups = data;
  updateGroupsDataAndDrawineLines("update");
}
